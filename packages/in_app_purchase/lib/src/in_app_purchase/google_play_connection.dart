...
/// common plugin API.
class GooglePlayConnection with WidgetsBindingObserver implements InAppPurchaseConnection {
  GooglePlayConnection._()
      : billingClient = BillingClient((PurchasesResultWrapper resultWrapper) async {
          _purchaseUpdatedController.add(await _getPurchaseDetailsFromResult(resultWrapper));
        }) {
  ...
  @override
  Future<BillingResultWrapper> completePurchase(PurchaseDetails purchase, {String developerPayload}) async {
    if (purchase.billingClientPurchase.isAcknowledged) {
      return BillingResultWrapper(responseCode: BillingResponse.ok);
    }
    return await billingClient.acknowledgePurchase(purchase.verificationData.serverVerificationData,
        developerPayload: developerPayload);
  }

  @override
  Future<BillingResultWrapper> consumePurchase(PurchaseDetails purchase, {String developerPayload}) {
  ...
  Future<QueryPurchaseDetailsResponse> queryPastPurchases({String applicationUserName}) async {
    ...
    try {
      responses =
          await Future.wait([billingClient.queryPurchases(SkuType.inapp), billingClient.queryPurchases(SkuType.subs)]);
    ...
    Set errorCodeSet = responses
        .where((PurchasesResultWrapper response) => response.responseCode != BillingResponse.ok)
        .map((PurchasesResultWrapper response) => response.responseCode.toString())
        .toSet();

    String errorMessage = errorCodeSet.isNotEmpty ? errorCodeSet.join(', ') : null;

    List<PurchaseDetails> pastPurchases = responses.expand((PurchasesResultWrapper response) {
    ...
    if (exception != null) {
      error = IAPError(
          source: IAPSource.GooglePlay, code: exception.code, message: exception.message, details: exception.details);
    } else if (errorMessage != null) {
      error = IAPError(source: IAPSource.GooglePlay, code: kRestoredPurchaseErrorCode, message: errorMessage);
    }

    return QueryPurchaseDetailsResponse(pastPurchases: pastPurchases, error: error);
  }

  @override
  Future<PurchaseVerificationData> refreshPurchaseVerificationData() async {
    throw UnsupportedError('The method <refreshPurchaseVerificationData> only works on iOS.');
  }
  ...
  Future<void> _connect() => billingClient.startConnection(onBillingServiceDisconnected: () {});
  ...
  Future<ProductDetailsResponse> queryProductDetails(Set<String> identifiers) async {
    ...
    try {
      responses = await Future.wait([
        billingClient.querySkuDetails(skuType: SkuType.inapp, skusList: identifiers.toList()),
        billingClient.querySkuDetails(skuType: SkuType.subs, skusList: identifiers.toList())
      ]);
    } on PlatformException catch (e) {
      exception = e;
      responses = [
        ...
        SkuDetailsResponseWrapper(
            billingResult: BillingResultWrapper(responseCode: BillingResponse.error, debugMessage: e.code),
            skuDetailsList: []),
        // ignore: invalid_use_of_visible_for_testing_member
        SkuDetailsResponseWrapper(
            billingResult: BillingResultWrapper(responseCode: BillingResponse.error, debugMessage: e.code),
    ...
    List<ProductDetails> productDetailsList = responses.expand((SkuDetailsResponseWrapper response) {
    ...
    Set<String> successIDS = productDetailsList.map((ProductDetails productDetails) => productDetails.id).toSet();
  ...
  static Future<List<PurchaseDetails>> _getPurchaseDetailsFromResult(PurchasesResultWrapper resultWrapper) async {
    IAPError error;
    ...
    final List<Future<PurchaseDetails>> purchases = resultWrapper.purchasesList.map((PurchaseWrapper purchase) {
      return _maybeAutoConsumePurchase(PurchaseDetails.fromPurchase(purchase)..error = error);
    }).toList();
    if (purchases.isNotEmpty) {
      return Future.wait(purchases);
    } else {
      return [
        PurchaseDetails(purchaseID: null, productID: null, transactionDate: null, verificationData: null)
  ...
  static Future<PurchaseDetails> _maybeAutoConsumePurchase(PurchaseDetails purchaseDetails) async {
    if (!(purchaseDetails.status == PurchaseStatus.purchased &&
        _productIdsToConsume.contains(purchaseDetails.productID))) {
      return purchaseDetails;
    }

    final BillingResultWrapper billingResult = await instance.consumePurchase(purchaseDetails);
